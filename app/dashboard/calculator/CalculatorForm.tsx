...
export function CalculatorForm({ calculateBattle, savePlan }: Props) {
  ...
  const handleSavePlan = () => {
    ...
    startSaveTransition(async () => {
      try {
        ...
        setSaveMessage(`Plan saved (ID: ${id})`);
      } catch (error) {
        const msg =
  ...
  const disableSave = isCalculating || isSaving;
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex flex-col gap-8 md:flex-row">
          ...
          <div className="flex-1">
            ...
            <TroopInputSection
              title="Infantry"
              typeKey="Infantry_enemy"
              form={form}
            />
            <TroopInputSection
              title="Vehicles"
              typeKey="Vehicles_enemy"
              form={form}
            />
            <TroopInputSection
              title="Distance"
              typeKey="Distance_enemy"
              form={form}
            />
            <TroopInputSection
              title="Battery"
              typeKey="Battery_enemy"
              form={form}
            />
          </div>
        </div>
        ...
        {saveMessage && (
          <p className="mt-2 text-xs text-emerald-400">{saveMessage}</p>
        )}
        {saveError && (
          ...
          <div className="space-y-1">
            <p>
              ...
              <span className="font-mono">
                {result.scoreYou.toFixed(2)}
              </span>
            </p>
            <p>
              Score Enemy:{" "}
              <span className="font-mono">
                {result.scoreEnemy.toFixed(2)}
              </span>
            </p>
            {details && (
              <>
                <p>
                  Engine:{" "}
                  <span className="font-mono">
                    {details.engineVersion ?? "n/a"}
                  </span>
                </p>
                <p>
                  WinIndex:{" "}
                  <span className="font-mono">
                    {typeof details.winIndex === "number"
                      ? details.winIndex.toFixed(3)
                      : "n/a"}
                  </span>
                </p>
              </>
